...
class CRUDUser:
    ...
    def set_user_roles(self, db: Session, user_id: int, role_ids: List[int]) -> List[Role]:
        current_ids = {
            row[0]
            for row in db.query(UserRole.role_id).filter(UserRole.user_id == user_id).all()
        }
        target_ids = set(role_ids)
        for role_id in current_ids - target_ids:
            self.remove_role(db, user_id=user_id, role_id=role_id)
        for role_id in target_ids - current_ids:
            self.assign_role(db, user_id=user_id, role_id=role_id)
